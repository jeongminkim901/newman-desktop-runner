...
function buildVariants({ queryParams, bodyJson, mode, customVariants }, maxVariants) {
  ...
  if (mode === "custom" && Array.isArray(customVariants)) {
    customVariants.forEach((v) => {
      if (!v || !v.label) return;
      if (v.query) variants.push({ label: v.label, query: v.query });
      else if (v.body) variants.push({ label: v.label, body: v.body });
    });
  ...
  if (queryParams && queryParams.length) {
    ...
    if (mode === "extended") {
      variants.push({ label: `body:long:${firstKey}`, body: { ...base, [firstKey]: "A".repeat(256) } });
      variants.push({ label: `body:special:${firstKey}`, body: { ...base, [firstKey]: "<script>alert(1)</script>" } });
      variants.push({ label: `body:sqli:${firstKey}`, body: { ...base, [firstKey]: "' OR 1=1 --" } });
    }
  }
