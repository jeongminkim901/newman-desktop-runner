...
function buildVariants({ queryParams, bodyJson, mode, customVariants }, maxVariants) {
  ...
  if (queryParams && queryParams.length) {
    for (const param of queryParams) {
      ...
      queryVariants.push({ label: `query:remove:${key}`, query: others, type: "query" });
      queryVariants.push({ label: `query:empty:${key}`, query: [ { key, value: "" }, ...others ], type: "query" });
      if (mode === "extended") {
        queryVariants.push({ label: `query:long:${key}`, query: [ { key, value: "A".repeat(256) }, ...others ], type: "query" });
        queryVariants.push({ label: `query:special:${key}`, query: [ { key, value: "' OR 1=1 --" }, ...others ], type: "query" });
      }
    }
  }

  if (bodyJson && bodyJson.json && Object.keys(bodyJson.json).length) {
    const base = bodyJson.json;
    for (const key of Object.keys(base)) {
      const removed = { ...base };
      delete removed[key];
      bodyVariants.push({ label: `body:remove:${key}`, body: removed, type: "body" });
      bodyVariants.push({ label: `body:empty:${key}`, body: { ...base, [key]: "" }, type: "body" });
      bodyVariants.push({ label: `body:null:${key}`, body: { ...base, [key]: null }, type: "body" });
      ...
      bodyVariants.push({ label: `body:type:${key}`, body: { ...base, [key]: broken }, type: "body" });
      if (mode === "extended") {
        bodyVariants.push({ label: `body:long:${key}`, body: { ...base, [key]: "A".repeat(256) }, type: "body" });
        bodyVariants.push({ label: `body:special:${key}`, body: { ...base, [key]: "<script>alert(1)</script>" }, type: "body" });
        bodyVariants.push({ label: `body:sqli:${key}`, body: { ...base, [key]: "' OR 1=1 --" }, type: "body" });
      }
    }
  }

  // Interleave query and body so both dimensions appear even with a small cap:
  // [q0, b0, q1, b1, q2, b2, ...]
  const merged = [];
  const len = Math.max(queryVariants.length, bodyVariants.length);
  for (let i = 0; i < len; i++) {
    if (i < queryVariants.length) merged.push(queryVariants[i]);
    if (i < bodyVariants.length) merged.push(bodyVariants[i]);
  }
...
function buildSecurityVariants({ queryParams, bodyJson, schema, mode = "basic" }, maxVariants) {
  const querySecVariants = [];
  const bodySecVariants = [];

  if (queryParams && queryParams.length) {
    const first = queryParams[0];
    querySecVariants.push({
  ...
  if (bodyJson && bodyJson.json && typeof bodyJson.json === "object" && !Array.isArray(bodyJson.json)) {
  ...
  // Interleave: [qs0, bs0, qs1, bs1, ...]
  const merged = [];
  const len = Math.max(querySecVariants.length, bodySecVariants.length);
  for (let i = 0; i < len; i++) {
    if (i < querySecVariants.length) merged.push(querySecVariants[i]);
    if (i < bodySecVariants.length) merged.push(bodySecVariants[i]);
  }
  return merged.slice(0, maxVariants);
}
